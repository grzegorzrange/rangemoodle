<?php
// Fix double-encoded UTF-8 ONLY in logstore_standard_log and notifications tables.
...
if (!CLI_SCRIPT) {
    ...
    echo '<html><head><meta charset="utf-8"><title>Fix Encoding - Logs</title></head><body><pre>';
}
...
echo "=== Fix Encoding: Logs & Notifications ===\n";
echo "Mode: " . ($dryrun ? "DRY RUN (preview)" : "FIX") . "\n\n";

// Double-encoded UTF-8 markers for Polish characters.
$markers = [
    'Ä…', 'Ä™', 'Å›', 'Å‚', 'Å¼', 'Åº', 'Ä‡', 'Å„', 'Ã³',
    'Ä„', 'Ä˜', 'Åš', 'Å»', 'Å¹', 'Ä†', 'Ã"',
];
...
$targets = [
    ...
    'notifications' => ['contexturlname', 'fullmessage', 'fullmessagehtml', 'smallmessage', 'subject'],
];
...
if ($dryrun) {
    ...
    if (CLI_SCRIPT) {
        echo "Aby naprawic: php fix_encoding_logs.php --fix\n";
    } else {
        $fixurl = new moodle_url('/fix_encoding_logs.php', ['fix' => '1', 'sesskey' => sesskey()]);
